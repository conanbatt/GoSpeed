...
test("Recording Mode", function() {
	...
	equal(this.rgf_player.recording, true, "RGFPlayer is in recording mode.");
	...
	equal(res, true, "Recording mode enabled successfuly.");
	equal(this.rgf_player.recording, false, "RGFPlayer is no longer in recording mode.");
	equal(this.rgf_player.isRecording(), false, "Not in recording mode.");
	equal(this.rgf_player.gospeed.mode, "rgf", "GoSpeed mode is RGF.");
	...
	equal(this.rgf_player.gospeed.get_pos(6, 8), undefined, "No stone has been placed as RGFPlayer is no longer in recording mode.");
});
